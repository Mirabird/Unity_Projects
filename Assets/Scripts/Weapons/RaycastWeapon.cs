...
public class RaycastWeapon : MonoBehaviour
{
    ...
    public ParticleSystem muzzleExplosionEffect;
    ...
    private AudioSource _weaponShoot;
    ...
    private void Awake()
    {
        ...
        _weaponShoot = GetComponent<AudioSource>();
    }
    ...
    {
        if (collision.gameObject.CompareTag("Destructible")) // у объектов есть тег "Destructible"
        {
    ...
    void RaycastSegment(Vector3 start, Vector3 end, Bullet bullet)
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, distance, layerMask))
        {
            ...
            if (hitInfo.collider.gameObject.CompareTag("Destructible"))     // Логика уничтожения
            {
                Destroy(hitInfo.collider.gameObject);

                // Добавляем эффект взрыва при попадании
    ...
    private void FireBullet(Vector3 target)
    {
        ...
        // Воспроизведение эффекта взрыва из дула
        if (muzzleExplosionEffect != null)
        {
            ParticleSystem muzzleExplosion = Instantiate(muzzleExplosionEffect, raycastOrigin.position, raycastOrigin.rotation);
            _weaponShoot.Play();
